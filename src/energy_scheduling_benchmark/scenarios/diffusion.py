...
async def execute_test_case(
    *,
    ...
    max_iter:
        Maximum diffusion iterations per generator.
    tol:
    ...
    grad_step = 0.5 * n_gens * eps.target_band / max(total_p_nom, 1.0)
...
def _add_diffusion_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        "--max-iter", type=int, default=2000, help="Maximum diffusion iterations."
    )
    parser.add_argument(
        "--tol",
        type=float,
...
def main(argv: list[str] | None = None) -> None:
    run_scenario_main(
        execute_test_case,
        doc=__doc__,
        default_name_base="diffusion",
        extra_args=_add_diffusion_args,
        extra_kwargs=("max_iter", "tol"),
        lossless_only=True,
