"""Tests for the exact-diffusion scenario.

End-to-end integration smoke tests of execute_test_case on the built-in toy
network, mirroring test_diffusion_scenario.py.
"""
...
from energy_scheduling_benchmark.scenarios.exact_diffusion import execute_test_case
...
class TestExecuteTestCase:
    ...
    async def test_run_writes_balanced_dispatch(self, tmp_path):
        ...
        name_base = str(tmp_path / "exact_diffusion")
    ...
    async def test_run_with_mean_metropolis_weight_rule(self, tmp_path):
        """The non-default weight rule must run end-to-end too (the scenario
        threads weight_rule through to every participant)."""
        scenario = build_toy_network(periods=24)
        name_base = str(tmp_path / "exact_diffusion_mm")
        await execute_test_case(
            scenario=scenario,
            delay_s=0.0,
            loss_percent=0.0,
    ...
    async def test_raises_on_lossy_transport(self, tmp_path):
        """Exact diffusion's fixed-round message accounting requires lossless comms."""
        scenario = build_toy_network(periods=24)
